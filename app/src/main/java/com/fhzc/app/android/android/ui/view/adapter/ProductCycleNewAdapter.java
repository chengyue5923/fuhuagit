 ...
 */
public class ProductCycleNewAdapter extends BasePlatAdapter<MyProductModel> {
    Context mContext;

    public ProductCycleNewAdapter(Context context) {
        super(context);
        mContext = context;
    }
    ...
    public View getView(int position, View itemView, ViewGroup parent) {
        ...
        if (itemView == null) {
            vh = new ViewHolder();
            itemView = LayoutInflater.from(context).inflate(R.layout.view_product_cycle_item_weath, null);
        ...
        try {
            ...
            if (!StringTools.isNullOrEmpty(dividends)) {
                ...
                int start = 1;
                int radius1 = CommonUtil.dip2px(context, 35);
                int radius2 = CommonUtil.dip2px(context, 5);
                for (int i = 0; i < strings.length; i++) {
                    int dValue = width - CommonUtil.dip2px(context, start * 70 + 35);
                    if (DateTools.getTime(strings[i], DateTools.DATE_FORMAT_STYLE_3) != model.getExpireDay() && DateTools.getTime(strings[i], DateTools.DATE_FORMAT_STYLE_3) != model.getFoundDay()) {
                        ...
                        int centre = ((int) (width * percent));
                        if (centre > dValue) {
                            centre = dValue;
                            start++;
                        }
                        layoutParams.setMargins(centre - radius1, 0, 0, 0);
...
//                            if(Math.round(toadyDifferFoundPercent*100)>)
                        //派息日到成立日
                        ...
                        pointgrayParams.setMargins(centre - radius2, 0, 0, 0);
